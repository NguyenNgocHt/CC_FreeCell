...
export class MainGame extends cc.Component {
    ...
    //singleTon
    private static instance: MainGame | null = null;
    ...
    public AnimateDeal(i: number)// hoạt động phát bài
    {
        ...
        position = this.Cells[n].TopCard() == null ? this.Cells[n].node.position : this.Cells[n].TopCard().node.position;
        cc.tween(this.cards[i].node)
            .to(0.01, { position: new cc.Vec3(position) })
            .call(() => {
